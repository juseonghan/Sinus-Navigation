...
ROS_IGTL_Test::ROS_IGTL_Test(int argc, char *argv[], const char* node_name)
{
  ...
  sub_polaris = nh->subscribe("/NDI/Endoscope/measured_cp", 10, &ROS_IGTL_Test::polarisCallback, this); 
...
void ROS_IGTL_Test::polarisCallback(const geometry_msgs::PoseStamped::ConstPtr& msg) {
...
void ROS_IGTL_Test::test_sending()
{
  ...
  // now i need to read from a ros node instead of textfile
  // not exactly sure what that entails, but have to start with making this a ros node that subscribes to the polaris node

  // std::ifstream myfile; 
  // myfile.open("/home/juseonghan/catkin_ws/src/ROS-IGTL-Bridge/testing/data/motion.txt"); 
  // if (myfile.is_open()){
  ...
  //   float tx,ty,tz,q0,q1,q2,q3; 
  //   myfile >> tx >> ty >> tz >> q0 >> q1 >> q2 >> q3; 

  //   ros_igtl_bridge::igtltransform transform_msg;
  //   transform_msg.transform.translation.x = tx;
  //   transform_msg.transform.translation.y = ty;
  //   transform_msg.transform.translation.z = tz;
  //   transform_msg.transform.rotation.x = q0;
  //   transform_msg.transform.rotation.y = q1;
  //   transform_msg.transform.rotation.z = q2;
  //   transform_msg.transform.rotation.w = q3;
  //   std::cout << "Transformation #" + std::to_string(count) << std::endl;
  //   std::cout << "tx: " << tx << " ty: " << ty << " tz: " << tz << std::endl;
  //   std::cout << "qx: " << q0 << " qy: " << q1 << " qz: " << q2 << " qw: " << q3 << std::endl; 
  //   transform_msg.name = "ROS_IGTL_POSE"; 

  //   transform_pub.publish(transform_msg); 
  //   count++; 

  //   // to simulate Polaris, pause for tenth of a second in between each pose reading
  //   ros::Duration(0.1).sleep();
  //}
